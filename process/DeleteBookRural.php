...
$selectBook= ejecutarSQL::consultar("SELECT * FROM rural WHERE CodigoLibro='".$primaryKey."'");
$fila=mysqli_fetch_array($selectBook, MYSQLI_ASSOC);
if(mysqli_num_rows($checkLoanBook)<=0){
    if(consultasSQL::DeleteSQL("rural", "CodigoLibro='$primaryKey'")){
        if($fila['Imagen']!=""){
        ...
        echo '<script type="text/javascript">
            swal({
                ...
                text:"Los datos del centro se eliminaron exitosamente",
                type: "success",
        ...
        echo '<script type="text/javascript">
            swal({
                ...
                text:"No se pudo eliminar el centro del sistema, por favor intenta de nuevo",
                type: "error",
                confirmButtonText: "Aceptar"
            });
        </script>';
    ...
    echo '<script type="text/javascript">
        swal({
            title:"¡Ocurrió un error inesperado!",
            text:"Este centro tiene préstamos registrados, no puedes eliminarlo",
            type: "error",
            confirmButtonText: "Aceptar"
        });
    </script>';
